...
export default class HorseDeliveryScene extends Phaser.Scene {
  ...
  // 1. ปรับท้องฟ้าให้เต็มจอ (ใช้ขนาด width, height ของ Canvas)
  this.bgFar = this.add.tileSprite(width / 2, height / 2, width, height, "horse-sky-pattern")
    .setDepth(-30)
    .setOrigin(0.5);

  // 2. ปรับพื้นหลังเลเยอร์ดินให้ชิดขอบล่าง
  this.groundLayer = this.add.tileSprite(width / 2, height, width, 210, "horse-ground-pattern")
    .setDepth(-10)
    .setOrigin(0.5, 1);

  this.groundEdgeGlow = this.add.rectangle(width / 2, height - 140, width, 26, 0xf3c06e, 0.14)
    .setDepth(-9);

  const skySource = this.textures.get("horse-sky-pattern")?.getSourceImage?.();
  if (skySource?.width && skySource?.height) {
    // คำนวณ Scale ให้ภาพท้องฟ้าขยายเต็มพื้นที่
    const scale = Math.max(width / skySource.width, height / skySource.height);
...
createBackdropSegments(width, height) {
  ...
  const targetHeight = height; // ปรับความสูงฉากหลังให้เท่าความสูงจอ
  let currentX = 0;

  this.backdropSegments = textureKeys.map((textureKey) => {
    const source = this.textures.get(textureKey)?.getSourceImage?.();
    const scale = source?.height ? targetHeight / source.height : 1;
    const displayWidth = Math.ceil((source?.width ?? width) * scale);
    
    const segment = this.add.image(currentX + (displayWidth / 2), height / 2, textureKey)
      .setOrigin(0.5)
      .setDepth(-20)
...
createStartOverlay() {
  ...
  // ลดขนาดป้ายเริ่มเกมลง (เดิม 0.92, 0.82)
  this.startCard = this.add.image(width / 2, height / 2, "horse-start-sign")
    .setDisplaySize(Math.min(width * 0.75, 750), Math.min(height * 0.65, 540));
    
  this.startButton = this.createOverlayButton("เริ่มเกม", () => this.startCountdown());
  // ขยับปุ่มขึ้นมา (เดิม +178)
  this.startButton.setPosition(width / 2, height / 2.2 + 100);
...
this.resultCard = this.add.image(width / 2, height / 2, "horse-result-sign")
    .setDisplaySize(Math.min(width * 0.70, 700), Math.min(height * 0.65, 500));
...
updateParallax(deltaSeconds) {
  ...
  // เลื่อนตำแหน่ง Image Segments และเช็คว่าถ้าหลุดจอซ้าย ให้ไปโผล่ต่อท้ายทางขวา
  if (this.backdropSegments?.length) {
    const speed = BACKDROP_SCROLL_SPEED * deltaSeconds * factor;
    let rightmostX = -Infinity;
    
    // หาตำแหน่งขวาสุดก่อน
    this.backdropSegments.forEach(s => { rightmostX = Math.max(rightmostX, s.x + s.displayWidth/2); });

    this.backdropSegments.forEach((segment) => {
      segment.x -= speed;
      // ถ้าภาพหลุดขอบซ้าย (x + halfWidth < 0)
      if (segment.x + (segment.displayWidth / 2) < 0) {
        segment.x = rightmostX + (segment.displayWidth / 2);
      }
    });
  }
}
